...
export const submitSearch = async (page: number, payload: SearchPayload, api: KyInstance): Promise<ProductList> => {
  const data = await api.post(`products/search/?page=${page}`, { json: payload }).json<ProductList>();
  return data;
};
